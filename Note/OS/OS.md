...
![计算机系统层次与操作系统的位置](./assets/01_os_layers.svg)
...
![应用通过操作系统使用硬件](./assets/01_app_os_hardware_path.svg)
...
![多道程序中的 CPU 与 I/O 重叠](./assets/01_cpu_io_overlap.svg)

早期系统一次只运行一道程序。程序发出磁盘 I/O 后，CPU 只能等待：

![单道程序 I/O 等待时 CPU 空闲](./assets/01_single_program_wait.svg)

多道程序设计把多个程序同时放入内存：

![多道程序在等待 I/O 时切换执行对象](./assets/01_multiprogram_switch.svg)
...
![单核 CPU 上的并发执行时间线](./assets/01_concurrency_timeline.svg)
...
![用户态、内核态与系统调用边界](./assets/01_user_kernel_boundary.svg)
...
![系统调用 read 的典型路径](./assets/01_system_call_steps.svg)
...
![中断处理流程](./assets/01_interrupt_entry.svg)
...
![典型中断处理流程](./assets/01_interrupt_steps.svg)
...
![进程三态模型](./assets/02_process_states.svg)
...
![进程实体的组成](./assets/02_process_entity.svg)
...
![进程创建流程](./assets/02_process_creation.svg)
...
![进程上下文切换](./assets/02_context_switch.svg)
...
![进程与线程的资源关系](./assets/02_process_thread.svg)
...
![临界区互斥访问](./assets/03_critical_section.svg)
...
![临界区代码的规范结构](./assets/03_critical_section_structure.svg)
...
![信号量数值的含义](./assets/03_semaphore_states.svg)
...
![用 P/V 表达前驱关系](./assets/03_predecessor_pv.svg)
...
![生产者消费者信号量](./assets/03_pv_buffer.svg)

### 6.1 为什么不能先 P(mutex) 再 P(empty)

假设缓冲区已满：

![先拿互斥锁再等空位可能导致死锁](./assets/03_wrong_p_order_deadlock.svg)
...
![死锁循环等待](./assets/03_deadlock_cycle.svg)
...
![死锁四个必要条件](./assets/03_deadlock_conditions.svg)
...
![银行家算法资源请求流程](./assets/03_banker_request_flow.svg)
...
![银行家算法安全性检查](./assets/03_banker_safety.svg)
...
![安全、不安全与死锁的关系](./assets/03_safety_state_relation.svg)
...
![资源分配图示意](./assets/03_resource_allocation_graph.svg)
...
![三级调度的作用范围](./assets/04_three_level_scheduling.svg)
...
![抢占式与非抢占式调度](./assets/04_preemption_compare.svg)
...
![调度时间指标](./assets/04_scheduling_metrics.svg)
...
![时间片轮转的三种结果](./assets/04_rr_outcomes.svg)

![RR 调度甘特图](./assets/04_rr_gantt.svg)
...
![甘特图记录每段开始和结束时刻](./assets/04_gantt_template.svg)
...
![抢占式优先级调度示例甘特图](./assets/04_priority_gantt_example.svg)
...
![内存管理的四类任务](./assets/05_memory_management_tasks.svg)
...
![逻辑地址到物理内存的转换路径](./assets/05_address_translation_path.svg)

## 3. 程序从磁盘进入内存的过程

![程序装入、链接与重定位](./assets/05_program_loading.svg)
...
![连续分配中的内存布局](./assets/05_contiguous_memory_layout.svg)
...
![内碎片与外碎片](./assets/05_fragmentation.svg)
...
![紧凑前后的空闲区变化](./assets/05_compaction.svg)

但紧凑需要动态重定位，且移动正在进行 I/O 的内存区域可能造成设备仍向旧地址传输，因此代价较大。

## 7. 动态分区分配算法

![动态分区分配算法对比](./assets/05_partition_algorithms.svg)
...
![分页中的逻辑页到物理页框映射](./assets/05_paging_mapping_example.svg)
...
![分页地址转换](./assets/05_page_translation.svg)
...
![分页地址中的页号与页内偏移](./assets/05_address_bits.svg)
...
![TLB 与分页地址转换](./assets/05_tlb_translation.svg)
...
![分段地址转换](./assets/05_segmentation.svg)
...
![段页式地址转换](./assets/05_segment_paging.svg)
...
![内存分配方式的整体关系](./assets/05_allocation_taxonomy.svg)

## 19. 地址转换题的固定流程

![地址转换题的固定流程](./assets/05_translation_checklists.svg)
...
![虚拟地址空间中的页面驻留状态](./assets/06_virtual_address_pages.svg)
...
![时间局部性与空间局部性](./assets/06_locality.svg)
...
![基本分页与请求分页](./assets/06_basic_vs_demand_paging.svg)
...
![缺页中断处理](./assets/06_page_fault.svg)
...
![页面置换算法对比](./assets/06_replacement_algorithms.svg)
...
![页面置换题的单列判断流程](./assets/06_page_replacement_decision.svg)
...
![Clock 页面置换算法](./assets/06_clock_algorithm.svg)
...
![抖动与工作集](./assets/06_thrashing_workingset.svg)
...
![设备分类的两个维度](./assets/07_device_classes.svg)
...
![逻辑设备名到物理设备的映射](./assets/07_logical_device_mapping.svg)
...
![一次文件读取 I/O 请求的路径](./assets/07_io_request_path.svg)

这条路径解释了为什么设备管理不是单一程序，而是多层软件和硬件协作。

## 5. I/O 控制方式为何不断演进

![I/O 控制方式演进](./assets/07_io_control_evolution.svg)
...
![DMA 成块传输时 CPU 与 DMA 并行](./assets/07_dma_timeline.svg)
...
![I/O 软件层次](./assets/07_io_stack.svg)
...
![驱动、控制器和实际设备的关系](./assets/07_driver_controller_device.svg)
...
![双缓冲吸收速度差异](./assets/07_buffering.svg)
...
![双缓冲交替工作](./assets/07_double_buffering.svg)
...
![设备分配路径](./assets/07_device_path_allocation.svg)
...
![直接占有打印机时的低效率](./assets/07_printer_occupy_timeline.svg)
...
![SPOOLing 流程](./assets/07_spooling.svg)
...
![CPU、DMA 和设备并行](./assets/07_parallel_io.svg)
...
![树形目录与索引分配](./assets/08_directory_index.svg)
...
![文件系统把名称逐步映射到磁盘设备](./assets/08_file_system_path.svg)
...
![文件抽象与磁盘块](./assets/08_file_abstraction.svg)
...
![文件的两种逻辑结构](./assets/08_logical_file_structures.svg)
...
![文件连续、链接与索引分配](./assets/08_allocation_methods.svg)
...
![三种文件物理分配示例](./assets/08_allocation_examples.svg)
...
![open(path) 建立快速访问连接](./assets/08_open_file_flow.svg)

之后，`read(fd, ...)` 和 `write(fd, ...)` 可通过 `fd` 直接找到打开文件表项，不必再次从完整路径开始查找。

### 12.1 两级打开文件表的直觉

![两级打开文件表](./assets/08_open_file_tables.svg)
...
![文件读取路径](./assets/08_file_read_path.svg)
...
![二级目录与树形多级目录](./assets/08_directory_trees.svg)
...
![文件多级索引](./assets/08_multilevel_index.svg)
...
![文件管理题的判断层次](./assets/08_file_question_map.svg)
