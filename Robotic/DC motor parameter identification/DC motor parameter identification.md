...
带入上面的力矩和电流的关系式 $T_e = K_tI$，我们可以得到角加速度 $\alpha$ 和电流 $I$ 的关系为：
...
$$
于是我们把上面那几坨全部无脑带入，就有了占空比 $D$  影响角速度 $\omega$ 的完整物理公式：
...
我们只需要知道 $\tau$ 和 $K$，就可以直接通过目标转速求解出电机占用比，以此实现在无负载条件下开环控制电机转速。
...
此时角加速度 $\dot{\omega} = 0$，我们就可以得到当时间 $t \to \infty$ 时的转速 $\omega_{\infty}$ 和电压 $U_s$ 满足关系：

$$
KU_s = \omega_{\infty}
$$

也就是只要给电机一个恒定的电压 $U_s$ （也就是恒定的占空比 $D$），通过编码器读数转速 $\omega_{\infty}$ 就能直接求出 $K$。
...
因此我们需要求出系统在给定电压 $U_s$ 后从 0 开始加速的这个过程中唯一的可观测量 $\omega$，也就是电机转速关于时间的表达式 $\omega(t)$。
...
$x$ 对 $t$ 求导，由于 $KU_s$ 在上文的恒定电压的场景下是一个常数，于是我们得到：

$$
\frac{dx}{dt}
=
\frac{d(KU_s-\omega)}{dt}
=
-\frac{d\omega}{dt}
$$
...
那么带入我们前面换元的 $x$ 的公式 $x = KU_s - \omega$，就有：

$$
x(0) = KU_s - \omega(0)
$$
毕竟 $K$ 和 $U_s$ 在我们上面提到的电压恒定的场景下都是一个常数。
...
x(0) = KU_s
$$
带回刚才我们解出的 $x(t) = C \mathrm{e}^{-t/\tau}$，就有：
...
$$
x(t) = KU_s \mathrm{e}^{-t/\tau}
$$

然后我们把 $x$ 的换元 $x(t) = KU_s - \omega(t)$ 再换回去：

$$
\omega(t) = KU_s - x(t)
= KU_s - KU_s \mathrm{e}^{-t/\tau}
= KU_s (1 - \mathrm{e}^{-t/\tau})
$$
...
于是我们就可以重新看回我们的问题：如何辨识 $\tau$ ？

我们发现，在 $t = \tau$ 时，公式就看起来非常好看了：

$$
\omega(\tau) 
= KU_s (1 - \mathrm{e}^{-\tau/\tau})
= KU_s (1 - \mathrm{e}^{-1})
$$

然后这个 $KU_s$ 我们利用系统在 $t = 0$ 时的状态也已经求出：
...
由于大部分电机都是运行在有负载工况下，我们可以比较无脑的直接对系统在有负载工况下进行参数辨识。虽然这和实际模型是有差别的，但是差别在负载固定且仅仅讲辨识参数用于计算速度环前馈的场景下几乎可以忽略。

无脑将 PI 速度环的输出和我们计算出的这个前馈输出相加，就可以得到经典的带有前馈的速度环方案了：

![带前馈的电机速度闭环](./assets/motor-speed-feedforward-loop.svg)

也就是说，对于目标恒定转速 $\omega_0$，此时目标加速度 $\dot{\omega_0} = 0$，我们可以直接带入电机模型，得到前馈电压 $U_{ff}$：
...
但是实际调试中这两个参数其实并不是十分好用，不如手调的好。加上推导过程比较简单，因此推导过程这里不做展开（其实是因为懒得写了）。